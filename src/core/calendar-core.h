/*
 * SPDX-License-Identifier: GPL-3.0-or-later
 * Copyright (C) 1993-2026 Shannon Smith
 *
 * Platform-neutral calendar engine contract.
 *
 * This header deliberately depends on GLib base types only.  It contains no
 * GObject, GVariant, Cinnamon or operating-system UI types, so another front
 * end can consume the same calendar model without adopting the GJS adapter.
 */

#ifndef CALENDAR_PLUS_CALENDAR_CORE_H
#define CALENDAR_PLUS_CALENDAR_CORE_H

#include "calendar-types.h"

G_BEGIN_DECLS

enum
{
    CALENDAR_PLUS_CALENDAR_GRID_CELLS = 42
};

typedef struct _CalendarPlusCalendarEngine CalendarPlusCalendarEngine;

/* Proleptic Gregorian civil date used at every platform boundary. */
typedef struct
{
    gint year;
    gint month;
    gint day;
} CalendarPlusDate;

typedef struct
{
    /* Owned UTF-8 label. calendar_plus_calendar_grid_clear() releases it. */
    gchar *day_label;
    /* Absolute proleptic-Gregorian coordinate for event and selection use. */
    CalendarPlusDate date;
    /* Table row 2..7 and logical weekday column 0..6. */
    gint row;
    gint column;
    /* ISO week number on the row's Thursday cell; zero on other cells. */
    gint week_number;
    gboolean is_work_day;
    gboolean is_today;
    gboolean is_selected;
    gboolean is_current_period;
    gboolean is_top_row;
    gboolean is_left_edge;
} CalendarPlusCalendarCell;

typedef struct
{
    CalendarPlusCalendarCell cells[CALENDAR_PLUS_CALENDAR_GRID_CELLS];
} CalendarPlusCalendarGrid;

typedef struct
{
    const gchar *id;
    const gchar *name;
} CalendarPlusCalendarDescriptor;

/**
 * calendar_plus_calendar_engine_new:
 * @calendar_id: stable provider id such as "gregorian"
 *
 * Resolves one immutable provider for formatting and navigation.
 *
 * Returns: (transfer full) (nullable): new engine, or %NULL for unknown id
 */
CalendarPlusCalendarEngine *calendar_plus_calendar_engine_new(
    const gchar *calendar_id);

/**
 * calendar_plus_calendar_engine_free:
 * @engine: (transfer full) (nullable): engine to release
 *
 * Accepts %NULL.
 */
void calendar_plus_calendar_engine_free(CalendarPlusCalendarEngine *engine);

/** Returns: (transfer none): process-lifetime stable provider id. */
const gchar *calendar_plus_calendar_engine_get_id(
    const CalendarPlusCalendarEngine *engine);
/** Returns: (transfer none): process-lifetime stable provider name. */
const gchar *calendar_plus_calendar_engine_get_name(
    const CalendarPlusCalendarEngine *engine);

/**
 * calendar_plus_calendar_engine_format_date:
 * @engine: calendar engine
 * @date: proleptic Gregorian coordinate
 * @part: requested provider date component
 *
 * Returns: (transfer full): allocated UTF-8 text; invalid input yields ""
 */
gchar *calendar_plus_calendar_engine_format_date(
    const CalendarPlusCalendarEngine *engine,
    const CalendarPlusDate *date,
    CalendarPlusDatePart part);
/**
 * calendar_plus_calendar_engine_period_start:
 * @engine: calendar engine
 * @date: source Gregorian coordinate
 * @result: (out): first Gregorian day of the provider period
 *
 * Writes @result only on success.
 */
gboolean calendar_plus_calendar_engine_period_start(
    const CalendarPlusCalendarEngine *engine,
    const CalendarPlusDate *date,
    CalendarPlusDate *result);
/**
 * calendar_plus_calendar_engine_add_periods:
 * @engine: calendar engine
 * @date: source Gregorian coordinate
 * @amount: signed provider-period delta
 * @result: (out): destination Gregorian coordinate
 *
 * Writes @result only on success.
 */
gboolean calendar_plus_calendar_engine_add_periods(
    const CalendarPlusCalendarEngine *engine,
    const CalendarPlusDate *date,
    gint amount,
    CalendarPlusDate *result);
/**
 * calendar_plus_calendar_engine_add_years:
 * @engine: calendar engine
 * @date: source Gregorian coordinate
 * @amount: signed provider-year delta
 * @result: (out): destination Gregorian coordinate
 *
 * Writes @result only on success.
 */
gboolean calendar_plus_calendar_engine_add_years(
    const CalendarPlusCalendarEngine *engine,
    const CalendarPlusDate *date,
    gint amount,
    CalendarPlusDate *result);
/**
 * calendar_plus_calendar_engine_period_key:
 * @engine: calendar engine
 * @date: source Gregorian coordinate
 *
 * Returns: (transfer full): allocated stable comparison key, or ""
 */
gchar *calendar_plus_calendar_engine_period_key(
    const CalendarPlusCalendarEngine *engine,
    const CalendarPlusDate *date);

/**
 * calendar_plus_calendar_engine_build_grid:
 * @engine: calendar engine
 * @selected: selected Gregorian date
 * @today: current Gregorian date
 * @week_start: 0=Sunday through 6=Saturday
 * @grid: (out): complete 42-cell model
 *
 * Fills all 42 cells on success. Each day_label is owned by @grid and released
 * by calendar_plus_calendar_grid_clear(). Invalid week starts fail.
 */
gboolean calendar_plus_calendar_engine_build_grid(
    const CalendarPlusCalendarEngine *engine,
    const CalendarPlusDate *selected,
    const CalendarPlusDate *today,
    gint week_start,
    CalendarPlusCalendarGrid *grid);
/**
 * calendar_plus_calendar_grid_clear:
 * @grid: (nullable): grid to clear
 *
 * Idempotently releases owned labels and zeroes the model.
 */
void calendar_plus_calendar_grid_clear(CalendarPlusCalendarGrid *grid);

/** Returns: number of immutable registered calendar providers. */
gsize calendar_plus_calendar_catalogue_get_count(void);
/**
 * calendar_plus_calendar_catalogue_get:
 * @index: zero-based provider index
 * @descriptor: (out): borrowed stable id/name descriptor
 *
 * Returns: %TRUE when @index names a registered provider.
 */
gboolean calendar_plus_calendar_catalogue_get(
    gsize index,
    CalendarPlusCalendarDescriptor *descriptor);

/** Returns: whether @date is a valid proleptic Gregorian civil date. */
gboolean calendar_plus_date_is_valid(const CalendarPlusDate *date);
/** Returns: whether two valid Gregorian field triples identify one day. */
gboolean calendar_plus_date_same(gint year_a,
                                 gint month_a,
                                 gint day_a,
                                 gint year_b,
                                 gint month_b,
                                 gint day_b);
/**
 * calendar_plus_date_is_work_day:
 * @year: Gregorian year
 * @month: Gregorian month 1..12
 * @day: Gregorian day
 *
 * Uses ICU/CLDR locale weekend policy, with Monday-Friday fallback only when
 * locale weekend information is unavailable.
 */
gboolean calendar_plus_date_is_work_day(gint year,
                                        gint month,
                                        gint day);

G_END_DECLS

#endif
