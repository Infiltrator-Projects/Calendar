...
Mint-only temporal fallback is explicitly guarded: if `system-settings` is absent, Calendar ignores any stale Infiltrator temporal policy file and delegates conventional clock/date presentation to Cinnamon's live desktop settings. The JavaScript runtime matrix covers 12/24-hour, date visibility, seconds and vertical-panel combinations; the month view separately follows Cinnamon's first-day-of-week helper and the event layer follows the operating-system timezone.\n\nThe local `.run` additionally performs a machine-specific two-pass PGO build: the instrumented native Calendar/Common build is exercised by the automated tests and a representative Calendar workload before the final profile-use rebuild. This is an optimisation process, not a substitute for correctness or live Cinnamon qualification.
