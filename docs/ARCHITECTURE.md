...
Within the arithmetic engine, the coordinator retains cross-family era mapping and generic navigation while the substantial Islamic/Umm al-Qura, Hebrew and Persian rule sets live in private sibling translation units. Each family keeps its month-length, conversion and family-specific navigation-sensitive logic together; the split follows chronology ownership rather than an arbitrary file-size target.
